...
add_filter('the_content','gSendtofriend_pageshow');

function gSendtofriend_pageshow($content)
{
	return 	preg_replace_callback('/\[SEND-LINK-TO-FRIEND=(.*?)\]/sim','gSendtofriend_pageshow_callback',$content);
}

function gSendtofriend_pageshow_callback($matches) 
{
	$sendlinks = "http://" . $_SERVER['HTTP_HOST']  . $_SERVER['REQUEST_URI'];
	$_sm = get_option('gSendtofriend_title_sm');
